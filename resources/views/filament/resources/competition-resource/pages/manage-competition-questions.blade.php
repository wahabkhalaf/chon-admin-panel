        ...
        <div class="p-6 bg-white rounded-xl shadow dark:bg-gray-800">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold tracking-tight">
                    Questions for {{ $record->name }}
                </h2>

                <div class="flex gap-2 items-center">
                    <span
                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                        Status: <span class="ml-1 capitalize">{{ $record->getStatus() }}</span>
                    </span>
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Competition opens: <span
                                class="font-medium">{{ $record->open_time->format('M d, Y H:i') }}</span>
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Competition starts: <span
                                class="font-medium">{{ $record->start_time->format('M d, Y H:i') }}</span>
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Questions attached: <span class="font-medium">{{ $record->questions()->count() }}</span>
                        </p>
                    </div>
                </div>
            </div>
